...
## Changelog and pre-releases (RC, beta, alpha)
...
ships. `cargo release --version 3.0.0-rc.1` does this automatically.
...
When pasting the changelog into the release body, unwrap the hard line breaks
so each bullet point is a single flowing paragraph — the fixed-width formatting
in `CHANGELOG.md` is for source readability, not the release page.
