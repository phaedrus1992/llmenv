...
fn generate_settings_json(out: &Path, manifest: &MergedManifest) -> anyhow::Result<()> {
    ...
    for hook in &manifest.capabilities.hooks {
        ...
        // Build the handler with only the keys that carry a value. A `json!`
        // object literal would serialize an absent `command`/`tool` as an
        // explicit `null`, which pollutes settings.json with null-valued hook
        // keys (#720 / the #699 no-null invariant). Omit them instead.
        let mut handler = serde_json::Map::new();
        handler.insert(
            "type".into(),
            json!(match hook.handler.kind {
        ...
        if let Some(tool) = &hook.handler.tool {
            handler.insert("tool".into(), json!(tool));
        }
...
mod tests {
    ...
    use super::{
        ...
        render_marketplace_source, render_permission_rule, seed_install_method,
    };
    ...
    // Bounded arbitrary JSON. Depth-limited so the recursive strategy stays
    // cheap; covers the value shapes reconcile_settings sees on disk and in a
    // fresh render (scalars, arrays, nested objects).
