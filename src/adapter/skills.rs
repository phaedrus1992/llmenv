...
/// symlinks) plus an explicit symlink rejection (#1337) closes half of that
/// window: a `src` root swapped for a symlink in that gap is rejected here
/// rather than followed. `copy_dir_owner_only` already rejects symlinks it
/// encounters *inside* the tree the same way; this closes the same gap at
/// the root. The other half — the source directory's *contents* still
/// changing between this check and the copy — is unclosed; see #1337 for
/// why (would need `openat`-style fd-relative I/O to close fully, judged
/// disproportionate for the residual risk).
///
/// # Errors
/// - Unsafe (path-traversal) skill name.
/// - Source path is a symlink, is not a directory, or doesn't exist.
...
/// loading the plugin.
///
/// Returns the paths written (relative to `out`) and the skill names
/// projected — the latter lets a caller (e.g. the output-style collision
/// check, #1333/#1335) learn exactly what was written without a second,
/// separate directory walk that could see a different result (TOCTOU).
/// Returns two empty vecs when `plugin_dir/skills/` does not exist.
///
/// # Errors
/// Propagates any I/O error from [`write_first_class_skills`].
pub(crate) fn project_plugin_skills(
    plugin_dir: &Path,
    out: &Path,
    ...
    let paths = write_first_class_skills(out, &skills)?;
    Ok((paths, names))
}
...
/// (which then writes them) and callers that need to discover a plugin's
/// skills before deciding whether to write them at all (opencode's
...
///
/// The field strategies come from `llmenv_util::testkit` (#1281): their
/// character classes deliberately include JSON/shell/template-escaping-
...
-> impl proptest::prelude::Strategy<Value = crate::config::HookHandler> {
    ...
    prop_oneof![
        llmenv_util::testkit::arb_hook_command_str().prop_map(|command| {
            crate::config::HookHandler {
                kind: crate::config::HookHandlerKind::Command,
                command: Some(command),
                tool: None,
            }
        }),
        llmenv_util::testkit::arb_hook_tool_str().prop_map(|tool| crate::config::HookHandler {
