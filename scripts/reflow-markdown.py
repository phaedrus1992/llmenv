...
def reflow_markdown(text: str) -> str:
    ...
    while i < len(lines):
        ...
        if (
            line.lstrip().startswith("#")  # heading
            or line.lstrip().startswith("|")  # table
            or line.lstrip().startswith(">")  # blockquote
            or line.startswith("    ")
            or line.startswith("\t")
        ):  # indented (code)
            result.append(line)
    ...
    if in_code_block:
        print(
            "reflow-markdown: unclosed code fence in input — aborting", file=sys.stderr
        )
        sys.exit(1)
