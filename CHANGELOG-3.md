...
- A tag from `.llmenv.yaml`'s `tags:` field or `$LLMENV_EXTRA_TAGS` containing anything outside alphanumeric/`-`/`_` used to pass through unnoticed until ICM's recall query rejected it — silently disabling memory recall/store *and* session logging for the rest of the session, with no visible error. Tags are now validated (and length- and count-capped) where they're created; invalid or excess tags are dropped with a warning instead. See [Configuration](https://phaedrus1992.github.io/llmenv/docs/configuration#project-markers) (#1035)
