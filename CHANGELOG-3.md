...
### Added
- Opt-in per-phase hook-run timing via `LLMENV_TRACE_TIMING` env var — emits a single `llmenv-trace {json}` stderr line with config-load/scope-eval/chunk-gen/mcp phase durations in microseconds; off by default, stdout unaffected
