...
- Warn about `native_<feature>.<engine>` keys no engine will ever read, instead of dropping them silently. A typo (`native_mcp.opencde`) or a key naming an engine that has no such feature (`native_model_providers.claude_code`) used to parse, merge, and hash cleanly and then vanish. `llmenv export`, `llmenv regenerate`, and `llmenv doctor` now all report both cases across every per-engine map. See [Engines](https://phaedrus1992.github.io/llmenv/docs/engines#engine-keys-are-validated) (#1032)
- `llmenv doctor` flags `capabilities.permissions` patterns that use Claude Code's colon-prefix syntax (`git commit:*`, `domain:example.com`) when opencode is also installed and enabled. opencode matches a pattern as a plain glob, so those grants are dead config there; the warning points at the portable form (`git commit *`) or at moving the Claude-only rule to `native_permissions.claude_code`. See [`doctor`](https://phaedrus1992.github.io/llmenv/docs/commands#doctor) (#838)

### Fixed

- `llmenv doctor` no longer reports `native_permissions.opencode` and `native_permissions.crush` as orphaned keys. The orphan check hardcoded `claude_code` as the only engine name, so the two newer adapters' own permission overrides were flagged even though both adapters read them. (#1032)
