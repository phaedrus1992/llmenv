...
- Rejecting a modeled key in `native.<engine>` told you to use `native_<key>.<engine>` as if that field always existed — for `provider`, `providers`, `model`, `models`, `small_model`, `lsp`, and `instructions` it never did. The error now names the escape hatches that actually exist. See [Engines](https://phaedrus1992.github.io/llmenv/docs/engines) (#1008)
