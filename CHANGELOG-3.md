...
- A `permissions.allow` rule with a `pattern` or `paths` scope (e.g. `Bash` + `git status:*`) now renders to Crush's `allowed_tools` as the bare tool name instead of a `tool(pattern)`/`tool(path)` entry. Source-verified against Crush's own matching code: `allowed_tools` only ever compares the bare tool name or a fixed `tool:action` string, never a command or path, so the scoped-looking entry was silently inert — it materialized into `crush.json` but never actually granted anything more (or less) than the bare tool would have. See [Engines](https://phaedrus1992.github.io/llmenv/docs/engines#the-crush-adapter) (#1306)
