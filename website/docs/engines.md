...
| --------- | ---------------- | ------- |
| Permissions (`allow`/`ask`/`deny`) | Supported | Rendered as per-tool `pattern → action` maps; a bare tool emits a plain action string. `ask` is native (no fail-closed collapse). The neutral tool name is mapped to opencode's own permission key — mostly a lowercase of the Claude Code name, but `Write`/`MultiEdit` both map to `edit` and `LS` maps to `list` (opencode has no separate `write`/`multiedit`/`ls` key; source-verified against opencode's `permission.ts` schema) (fixed in v3.10.0, [#1326](https://github.com/phaedrus1992/llmenv/issues/1326)) |
