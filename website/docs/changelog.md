...
- `llmenv setup` no longer skips engines added after it was written; `compute_disabled_engines` hardcoded the same three-engine list rather than reading the adapter registry, so a new adapter wouldn't be offered by the wizard and `disabled_engines` would be computed against a stale universe. Same bug class as #1032. (#1074)
