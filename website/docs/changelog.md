...
- First-class `llmenv statusline` subcommand: reads engine session JSON from stdin, config from the new `statusline:` section of `config.yaml`, and llmenv's own stats (active scopes, plugin/MCP counts, ICM memory stats, throttle state, cache health, config staleness, session log activity) from a materialized `llmenv-status.json` data file. Supports 21 widget types — 13 engine-sourced (`model`, `folder`, `branch`, `pr`, `progress_bar`, `tokens`, `context_pct`, `budget`, `duration`, `cache_pct`, `usage_5h`, `usage_7d`, `peak`) and 8 llmenv-sourced (`scopes`, `plugins`, `mcps`, `icm`, `cache`, `config_stale`, `throttle`, `session_log`) — with per-widget `format`/`style`/`max_len` overrides, configurable row templates, and a configurable icon set (`auto`/`nerd`/`simple`/`none`). Widgets carry sensible **default colors** so a bare `statusline:` block reads legibly rather than monochrome; opt out per widget with `style: none` or globally with `statusline.style.color: false`. Default glyphs match the reference layout (📁 folder, 🌿 branch, ⏱ duration, ↻ cache_pct, ▓ progress bar, ⚙️ config_stale) and subdued widgets use a readable light gray rather than hard-to-read faint `dim`; `model` offers `short`/`version`/`full` display modes; `progress_bar` takes a configurable cell `width` and per-cell threshold coloring (filled cells in the usage color, empty cells dim); and `duration` shows seconds under an hour (`45s`, `2m 30s`). `usage_5h`/`usage_7d` render the Claude.ai 5-hour/7-day rate-limit windows from the session's `rate_limits` — used %, a per-cell bar (filled in the threshold color, empty dim) with a bright pace-target marker, reset countdown, an over/under-pace indicator (`⇡`/`⇣`), a since-last-render delta (`(+4.5)`, persisted under `$CLAUDE_CONFIG_DIR/statusline-state/`), and threshold coloring — and `peak` shows the peak/off-peak billing window (`△ peak 3h03m`) computed from the local clock (Anthropic's weekday 05:00–11:00 Pacific window, no stdin data needed). The `pr` widget adds a `url` display mode and colors by `review_state` (approved green, changes-requested red, pending yellow); `pr` and `branch` render as OSC 8 terminal hyperlinks to the PR when a URL is present (validated `http`/`https`, control-char-free). Untrusted free-text (model/folder/branch names, PR URL, tags, throttle backend) is control-char-sanitized at each widget's interpolation point so a hostile directory or branch name cannot inject terminal escapes. The `branch` widget resolves the branch from git (`.git/HEAD`, following a worktree `.git`-file pointer) because Claude Code does not send a branch on stdin for regular repos; the `model` widget strips a trailing `(1M context)`-style qualifier and derives the version from `display_name` when the engine sends no separate `version`. The Claude Code adapter seeds `llmenv statusline` as the default `statusLine` hook in `settings.json` automatically, without overwriting an existing user customization. Crush has no statusline hook concept yet (#855 tracks adding it). Full feature parity with the reference `rusty-claude-status` layout is tracked in #895 (#836)
