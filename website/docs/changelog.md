...
- A Codex adapter, PATH-gated like the others: with `codex` on `PATH`, llmenv materializes `config.toml` into a `codex/` subtree and exports `CODEX_HOME`. This first slice renders MCP servers and the merged `AGENTS.md` (via `model_instructions_file`); permissions, lifecycle hooks, statusline, auth, plugins, and doctor checks are tracked separately and are listed with their issue numbers in the docs rather than failing quietly. An MCP server using the SSE transport is skipped for Codex with a warning — Codex speaks stdio and streamable HTTP only, and rendering SSE as a URL would produce a server it can't talk to. See [Engines](https://phaedrus1992.github.io/llmenv/docs/engines#the-codex-adapter) (#233)
