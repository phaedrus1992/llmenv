...
- `llmenv launch --container` now verifies a pulled sandbox image's build-provenance attestation with `gh attestation verify` before running it, and refuses to run an image that fails verification. A machine with no `gh` (GitHub CLI) installed, or no network path to GitHub, still launches — verification is skipped with a one-line stderr notice rather than blocking, since only a `gh` that actually reached GitHub and reported a failure is treated as a real signal. See [Commands](https://phaedrus1992.github.io/llmenv/docs/commands#sandbox-featuressandbox) (#1719)
