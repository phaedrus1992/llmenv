...
- Materializing bundle files into an existing materialized folder no longer writes through a pre-existing symlink at the destination — a plain file copy opens the destination with `O_TRUNC`, which follows a symlink there and overwrites whatever it points at instead of replacing the link. Affected `llmenv export`/`regenerate`'s re-render of bundle content for Claude Code and every other adapter that shares `write_in_place`. The copy now lands in a same-directory temp file first, then atomically renames over the destination — POSIX's rename semantics replace the destination entry regardless of what it is, without ever following it (#1422, #1423)
