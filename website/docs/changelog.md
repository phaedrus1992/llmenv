...
- Add `capabilities.native_model_providers.<engine>`, the missing escape hatch for provider config on opencode and Crush. Both adapters reject `provider`/`providers` in the top-level `native.<engine>` block, and `capabilities.model_providers` models only the fields the engines have in common — so an engine-specific provider or per-model key (opencode's per-model `reasoningEffort`, say) had nowhere to go. The new per-engine fragment deep-merges onto the rendered provider block at higher precedence, and works with no `model_providers` entries at all, so a hand-written provider survives `llmenv regenerate`. See [Engines](https://phaedrus1992.github.io/llmenv/docs/engines#native_model_providersengine) (#1008)
...
- Rejecting a modeled key in `native.<engine>` told you to use `native_<key>.<engine>` as if that field always existed — for `provider`, `providers`, `model`, `models`, `small_model`, `lsp`, and `instructions` it never did. The error now names the one escape hatch that applies to the rejected key, or says there is none and points at the neutral `capabilities` field instead. See [Engines](https://phaedrus1992.github.io/llmenv/docs/engines) (#1008)
- A `native_*.<engine>` fragment that wasn't a mapping — most often a YAML indentation slip, or a key left with an empty body — silently deleted the entire block it was meant to merge into, taking any neutrally-declared config in that block with it, and exited 0. Affected `native_mcp`, `native_hooks`, and the top-level `native.<engine>`. It now fails with an error naming the field and the shape it actually got. See [Engines](https://phaedrus1992.github.io/llmenv/docs/engines) (#1008)
