...
- `write_owner_only` — the single most widely-used config/state writer, called from every adapter for `settings.json`, `AGENTS.md`/`CLAUDE.md`, hook JSON, and more — no longer writes through a pre-existing symlink at its destination. Same root cause as the two entries above (`.create(true).truncate(true)` follows a symlink and truncates its target), fixed the same way: unlink first, then create exclusively, matching the contract `write_owner_only_atomic` already had via its temp-file-plus-rename approach (#1429)
