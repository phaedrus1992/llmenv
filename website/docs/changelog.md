...
- A skill source path that changes filesystem identity (a symlink swap, detected via dev/inode comparison, Unix only) during its copy now discards the copy and fails, instead of trusting content the initial symlink check never actually validated. A symlink already present at a materialized output path llmenv owns (e.g. `skills/<name>`) is now rejected instead of followed — previously, `create_dir_owner_only` would chmod the symlink's *target* to owner-only and write through it. A symlinked `SKILL.md` inside a skill's source is now a hard error instead of being silently dropped from the copy (which used to produce a confusing "missing SKILL.md" failure later); any other symlinked file inside a skill's source is still skipped, now with a visible warning instead of a debug-only log line. `llmenv`'s own state-directory inheritance (`history.jsonl`, the MCP needs-auth cache) applies the same never-follow-a-symlink check. (#1341)
