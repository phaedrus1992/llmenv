...
- Extends the fix above to *directory* components, not just the final file: `write_in_place` and the Claude Code adapter's `materialize` no longer follow a symlinked subdirectory anywhere along a bundle file's relative path. `create_dir_all(parent)` happily resolves through a symlinked intermediate directory, which is a strictly stronger primitive than the leaf-only bug — write content into any directory the invoking user can write, under a name of the attacker's choosing — so both call sites now walk each directory component via `openat`-relative descent (the same primitive `llmenv prune`'s stranded-transcript cleanup already uses) instead of resolving the whole path at once (#1427)
