...
- `llmenv launch`'s mid-session notice socket now checks the connecting peer's uid and rejects anything not running as the same user, closing a gap the socket's own introduction (v4.0.0) left open: making the socket's directory and file owner-only stopped a different local user from reaching the socket at all, but any process running as the same user could still connect and inject a fabricated notice into the agent's context. No new dependency — `tokio::net::UnixStream::peer_cred()` and `rustix::process::geteuid()` were both already in use. See [Commands](https://phaedrus1992.github.io/llmenv/docs/commands#launch) (#1483)
